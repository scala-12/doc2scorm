...
public class TableBlockTest extends AbstractBlockTest {
	...
	public void toHtml() {
		for (XWPFTable table : TestUtils.getTestTables()) {
			...
			assertEquals(htmlTBody.getChildNodes().getLength(), block
					.getItems().size());

			for (int rowNum = TestUtils.CONTENT_FROM_ROW; rowNum < htmlTBody
					.getChildNodes().getLength(); rowNum++) {
				...
				for (int cellNum = TestUtils.CONTENT_FROM_COLUMN; cellNum < htmlRow
						.getChildNodes().getLength(); cellNum++) {
					CellItem cell = row.get(cellNum).getFirstItem();
					Element htmlCell = (Element) htmlRow.getChildNodes().item(
							cellNum);
					...
					assertEquals(htmlCell.getAttribute("colspan"),
							(cell.getColSpan() == 1) ? "" : cell.getColSpan()
									.toString());
					assertEquals(htmlCell.getAttribute("rowspan"),
							(cell.getRowSpan() == 1) ? "" : cell.getRowSpan()
									.toString());

					if (cell.getValue() == null) {
						assertTrue(htmlCell.getTextContent().isEmpty());
					} else {
						for (AbstractParagraphBlock<?> content : cell
								.getValue()) {
							if (content instanceof ParagraphBlock) {
								assertEquals(htmlCell.getTextContent(),
										((ParagraphBlock) content).getText());
							}
						}
					}
				}
