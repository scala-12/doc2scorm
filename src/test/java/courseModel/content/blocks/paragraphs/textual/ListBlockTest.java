...
public class ListBlockTest extends AbstractBlockTest {
	...
	public void toHtml() {
		...
		for (List<XWPFParagraph> list : TestTools.getTestListsParagraphs()) {
			...
			if (block.getMarkerType() == MarkerType.SIMPLE_MARKER) {
				assertEquals(htmlBlock.getNodeName().toLowerCase(), "ul");
			} else {
				assertEquals(htmlBlock.getNodeName().toLowerCase(), "ol");

				if (block.getMarkerType() == MarkerType.UPPER_LETTER_MARKER) {
					assertEquals(htmlBlock.getAttribute("type"), "A");
				} else if (block.getMarkerType() == MarkerType.LOWER_LETTER_MARKER) {
					assertEquals(htmlBlock.getAttribute("type"), "a");
				} else if (block.getMarkerType() == MarkerType.UPPER_ROMAN_MARKER) {
					assertEquals(htmlBlock.getAttribute("type"), "I");
				} else if (block.getMarkerType() == MarkerType.LOWER_ROMAN_MARKER) {
					assertEquals(htmlBlock.getAttribute("type"), "i");
				} else if (block.getMarkerType() == MarkerType.DECIMAL_MARKER) {
