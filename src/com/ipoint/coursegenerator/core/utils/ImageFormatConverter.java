...
public class ImageFormatConverter {
	...
	public static byte[] transcodeEMFtoPNG(byte[] data) {
		...
		try {
			 ...
			 * Node defs = html.getElementsByTagName("defs").item(0);
			 * doc.getElementsByTagName("svg").item(0)
			 * .appendChild(doc.importNode(defs, true)); ((Element)
			 * doc.getElementsByTagName("g").item(0)).setAttribute( "style",
			 * "font-family:OpenSymbol; fill:black;"); NodeList nl =
			 * doc.getElementsByTagName("text"); for (int i = 0; i <
			 * nl.getLength(); i++) { nl.item(i).setTextContent( nl.item(i)
			 * .getTextContent() .replaceAll( new String(new byte[] { (byte)
			 * 0xC2, (byte) 0xB3 }, "UTF-8"), "\u2265"));
			 * nl.item(i).setTextContent( nl.item(i) .getTextContent()
			 * .replaceAll( new String(new byte[] { (byte) 0xC2, (byte) 0xB6 },
			 * "UTF-8"), "\u2202")); nl.item(i).setTextContent( nl.item(i)
			 * .getTextContent() .replaceAll( new String(new byte[] { (byte)
			 * 0xC3, (byte) 0xB2 }, "UTF-8"), "\u222b")); }
			 */
			// StreamResult sr = new StreamResult(out);
			// getTransformer().transform(new DOMSource(doc), sr);
			...
			// Map<TranscodingHints.Key, Object> hints = new
			// HashMap<TranscodingHints.Key, Object>();
			// hints.put(PNGTranscoder.KEY_XML_PARSER_VALIDATING, new Boolean(
			// false));
			// transcoder.setTranscodingHints(hints);
