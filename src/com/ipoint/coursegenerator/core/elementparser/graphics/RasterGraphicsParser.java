...
public class RasterGraphicsParser extends AbstractGraphicsParser {
	...
	if (pic.getMimeType().equals(IMAGE_PNG)) {
	    String url = "img" + File.separator + FileWork.IMAGE_PREFIX
		    + pic.hashCode() + ".png";
	    FileWork.savePNGImage(pic.getRawContent(), path + File.separator
		    + url);
	    imgElement
    ...
    public static void parse(XWPFPictureData pic, String path,
	    Element imgElement) {
	String url = null;
	...
	if (url != null) {
	    FileWork.savePNGImage(pic.getData(), path + File.separator + url);
	    imgElement
		    .setAttribute("src", url.replace(File.separatorChar, '/'));
	}
    }
